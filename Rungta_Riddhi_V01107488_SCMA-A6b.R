...
lt_data <- read.csv(file_path)

# View the first few rows of the data
head(lt_data)

# Calculate returns (log returns or percentage returns)
lt_data$Returns <- c(NA, diff(log(lt_data$Close)) * 100)

# Remove the first row with NA value
lt_data <- lt_data[-1, ]

# Check for ARCH effects
arch_test <- ArchTest(lt_data$Returns, lags = 12)
print(arch_test)
...
fit <- ugarchfit(spec = spec, data = lt_data$Returns, solver = "hybrid")
...
plot(forecasted_sigma, type = "l", main = "Forecasted Volatility", xlab = "Days Ahead", ylab = "Sigma")
...
columns_to_keep <- c('Date', 'Soybeans', 'Soybean oil', 'Soybean meal')
...
write.csv(df, 'Soybean_commodity_prices.csv', row.names = FALSE)
